...
if not has_access:
    st.set_page_config(page_title="접근 제한", layout="centered")
    st.error("🛑 접근 권한이 없습니다.")
    st.info("이 프로그램에 접근할 수 있는 승인된 구글 계정이 아닙니다. 관리자에게 문의하세요.")
    st.stop()
...
st.set_page_config(page_title="알바 급여 관리자", layout=True if st.responsive else "wide")
